...
void    privateMsg(Command& cmd, Client &user, std::vector<Channel> &channels,std::vector<Client> &clients)
{
    if (cmd.params.size() > 0)
    {
    	...
    	while (i < cmd.users.size())
    	{
				...
				if (it != channels.end())
				{
					for (size_t i = 0; i < it->userlist.size(); ++i)
					{
						if (it->userlist.at(i).clientFd != user.clientFd)
							write_to_client(it->userlist.at(i).clientFd, PRIVMSG(user.nickName, user.userName, user.ipaddr, it->getname(), cmd.message));	
					}
				}
				else
			...
			else // user
			{
				...
				if (it != clients.end())
					write_to_client(it->clientFd, PRIVMSG(user.nickName, user.userName, user.ipaddr, it->nickName, cmd.message));
				else
        ...
        write_to_client(user.clientFd, ERR_NORECIPENT(user.nickName, "(PRIVMSG)"));
}